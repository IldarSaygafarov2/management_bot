...
def get_lastname(message: types.Message, first_name: str):
    ...
    bot.send_message(chat_id, text=messages.USER_COURSE_MESSAGE, reply_markup=kb.show_courses_menu())
...
def get_course(message: types.Message, first_name: str, last_name: str):
    chat_id = message.from_user.id
    course = message.text
    bot.send_message(chat_id, text=messages.USERS_TEACHER_MESSAGE, reply_markup=kb.show_teachers_menu())
    bot.register_next_step_handler(message, get_teacher, first_name, last_name, course)


def get_teacher(message: types.Message, first_name: str, last_name: str, course: str):
    chat_id = message.from_user.id
    print(first_name, last_name, course, message.text)
